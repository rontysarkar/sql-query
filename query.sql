...
-- =========================================================================
create type payment_status as enum('Refunded','Cancelled','Confirmed','Pending');
...
select user_id,full_name,email from users where full_name ilike any (array['%Haque%','Tanvir%'])


-- 3) Retrieve all booking records where the payment status is missing (NULL), replacing the empty result with 'Action Required'.
select booking_id,user_id,match_id,coalesce(payment_status::text,'Action Required') as "systematic_status" from bookings where payment_status is null


-- 4) Retrieve match booking details along with the User's full name and the scheduled Match fixture teams.

select booking_id,full_name,fixture,round(total_cost) from bookings inner join users using(user_id) inner join matches using(match_id)
...
select booking_id,match_id,round(total_cost) from bookings where total_cost > (select avg(total_cost) from bookings)
